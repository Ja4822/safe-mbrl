...
def run(logger, config, args):
    ...
    elif args.robot.lower() == "limo":
        env_config = DEFAULT_ENV_CONFIG_LIMO
    # env = SafetyGymEnv(robot=args.robot, task="goal", level=args.level, seed=args.seed, config=env_config)
    env = GazeboCarNavEnv(robot=args.robot, task="goal", level=args.level, seed=args.seed, config=config["env_config"])
